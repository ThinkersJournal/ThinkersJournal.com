...
- **PARTLY RESOLVED 2026-09-05** — ~~Create the actual **Open Collective / GitHub Sponsors / Ko-fi** accounts and wire real links (placeholders until then).~~ **Open Collective and Ko-fi are live** and their links resolve (`SUPPORT` in `src/consts.ts`, `live: true`); **Liberapay went live 2026-09-08** (PR #18) as the recurring, zero-commission avenue — handle `ThinkersJournal.com`, linking to `/donate`. Its `button.js` widget was declined on inspection: a `document.write()`-emitted yellow Helvetica button showing no donation total, i.e. no benefit over the card, against a parser-blocking third-party script, a palette clash, and a permanent `script-src https://liberapay.com` widening of a CSP we intend to promote to enforcing. **GitHub Sponsors is now the only one still pending** — blocked ~2 months with no support channel available on a free account — and stays hidden rather than rendered as a dead link. `tests/pages.spec.ts` covers both states.
- ~~Founder's full **About** content for `/mission` (user to provide).~~ **RESOLVED 2026-09-08** — the founder supplied the substance and it shipped as the *"Why I'm building this"* section on `/mission` (PR #19): being helped by programmers further down the path, paying it forward, presence over brilliance, and the reason the help is free — the people who need it most are least able to buy it, some broke and others unwilling to spend on themselves because they don't yet believe they're worth it. Signed *"— the founder"*; **whether to use his real name remains his call.**
...
- ~~Custom **logo** (typographic wordmark until then).~~ **RESOLVED 2026-09-08** — shipped in PR #13: an open journal with a bookmark, black on brand green, generated by `scripts/generate-logo.mjs` into six assets: `favicon.svg`, `favicon-32.png`, `favicon-16.png`, `apple-touch-icon.png` (opaque — iOS renders transparency as black), `logo-192.png` and `logo-512.png`. **It did not replace the wordmark:** the mark supplies the icon set, while `Wordmark.astro` remains the in-page brand lockup, so both are current and neither is a placeholder.
