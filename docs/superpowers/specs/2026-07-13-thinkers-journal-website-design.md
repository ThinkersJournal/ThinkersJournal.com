...
- ~~Custom **logo** (typographic wordmark until then).~~ **RESOLVED 2026-09-08** — shipped in PR #13: an open journal with a bookmark, black on brand green, generated by `scripts/generate-logo.mjs` into `favicon.svg`, `favicon-32/16.png`, `apple-touch-icon.png` (opaque — iOS renders transparency as black) and `logo-192/512.png`. **It did not replace the wordmark:** the mark supplies the icon set, while `Wordmark.astro` remains the in-page brand lockup, so both are current and neither is a placeholder.
