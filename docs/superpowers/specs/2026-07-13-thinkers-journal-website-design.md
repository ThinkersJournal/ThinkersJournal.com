...
- **PARTLY RESOLVED 2026-09-05** — ~~Create the actual **Open Collective / GitHub Sponsors / Ko-fi** accounts and wire real links (placeholders until then).~~ **Open Collective and Ko-fi are live** and their links resolve (`SUPPORT` in `src/consts.ts`, `live: true`); **GitHub Sponsors is the only one still pending** — blocked ~2 months with no support channel available on a free account — and is hidden from the site rather than rendered as a dead link. **Liberapay was added 2026-09-08** (PR #18) as the recurring, zero-commission avenue, likewise gated until the account exists. `tests/pages.spec.ts` covers both states.
